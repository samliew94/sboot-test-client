<script lang="ts">
  ...
  async function onSave() {
    const uri = `${Config.origin}/api/products/${productCode}`;
  ...
  async function findByProductCode(){

    const uri = `${Config.origin}/api/products/${productCode}`;

    const res = await fetch(uri);

    inputFields = await res.json()



  }

  onMount(findByProductCode);

</script>

{#if show}
  <!-- svelte-ignore a11y-click-events-have-key-events -->
  <div class="p-4 rounded-xl bg-white p-4 rounded-xl bg-white border border-black border-2 border-gray-500">
    <p class="text-2xl mb-2">Edit Product</p>
    <div class="grid grid-cols-1 gap-1 flex justify-center items-center">
    
    {#each errors as error}
        <p class='text-red-500 text-sm italic'>{error}</p>
    {/each}
    <MyInput inputFields={inputFields} name="code" limit={9} title="Code" required={true} disabled={true}/>
    ...
    <div class="text-center text-white m-2">
      <MyButton title="Save" color="blue" onClick={onSave} />
      <MyButton title="Cancel" color="red" onClick={()=>(show=false)} />
      <MyButton title="Delete" color="red" onClick={onDelete} base={500} />
    </div>
  </div>
{/if}
